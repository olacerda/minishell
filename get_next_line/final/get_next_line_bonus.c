...
/*   Updated: 2026/01/04 23:28:54 by otlacerd         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	*(t_w *)&x[F] = (t_w){0, -1, 0, 0, 0, x[F].e + ((x[F].b[x[F].e]) == 10)};
	...
	((F < 0 || BUFFER_SIZE <= 0 || BUFFER_SIZE > MAX)) && (x[F].rb = 0);
	while ((x[F].l == NULL || x[F].b[x[F].e] != 10) && x[F].rb > 0)
	{
		(x[F].s >= x[F].rb) && (x[F].rb = read(F, x[F].b, BUFFER_SIZE));
		(x[F].s >= x[F].rb) && (*(long *)&x[F].e = 0);
		while ((x[F].e < x[F].rb) && (x[F].b[x[F].e] != 10) && ++(x[F].e))
			++(x[F].a);
		(x[F].rb > 0) && (x[F].n = malloc(x[F].a + 1 + (x[F].b[x[F].e] == 10)));
		(!x[F].n) && (x[F].rb = -2);
		while (++x[F].i < (x[F].a - x[F].e) && x[F].l && x[F].rb > 0)
			x[F].n[x[F].i] = (x[F].l)[x[F].i];
		while ((x[F].s <= x[F].e) && (x[F].s < x[F].rb) && x[F].rb > 0)
