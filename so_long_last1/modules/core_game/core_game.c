...
/*   Updated: 2025/11/07 04:16:21 by otlacerd         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	terminal_management(t_all *all)
{
	if (all->terminal->close_request && all->window_terminal)
	{
		mlx_destroy_window(all->mlx, all->window_terminal);
		key_ctrl_c(all);
		all->window_terminal = NULL;
		all->terminal->close_request = 0;
	}
