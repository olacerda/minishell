...
/*   Updated: 2025/10/19 18:02:48 by otlacerd         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	if (cmp_msg(all, "free(memory)", writed[*line], 12) == 0)
	{
				...
				|| all->states->undefined_behavior > 0))
		{
			all->map->map[all->game->element[indexor("E")].line][
				all->game->element[indexor("E")].column] = '0';
			free_memory(all);
		}
...
void	message_malloc(t_all *all, int *line, char (*writed)[33], int size)
{
	if (size == 18 || size == 19)
	{
		if (((writed[*line][all->terminal->pc_nb_size + 16] - 48) <= 5)
			&& (!(writed[*line][all->terminal->pc_nb_size + 17] >= 48 && writed[
				*line][all->terminal->pc_nb_size + 17] <= 57)
