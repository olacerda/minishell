...
/*   Updated: 2025/10/19 19:30:27 by otlacerd         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	key_enter(t_all *all, int *line, int *index)
{
	...
	flush_strings_on_terminal(all, *line, all->images->blank->st);
	...
	if (all->terminal->wr_index < 27 && all->states->key_shift == 1)
	{
		if (code == 57)
		{
			...
			mlx_put_image_to_window(all->mlx, all->window_terminal,
				all->images->blank->st, 10 + ((*index) * 6), 142);
		...
		if (code == 48)
		{
			...
			all->terminal->writed[line][*index + 2] = '\0';
			mlx_put_image_to_window(all->mlx, all->window_terminal,
				all->images->blank->st, 11 + ((*index) * 6), 142);
			mlx_string_put(all->mlx, all->window_terminal, 10, 160, 16711680,
				all->terminal->writed[line]);
			(*index)++;
		}
...
int	key_ctrl_c(t_all *all)
{
	...
	all->terminal->string_focused = all->terminal->wr_line;
	if (all->terminal->close_request != 1)
	{
...
int	close_terminal_window(t_all *all)
{
	all->terminal->close_request = 1;
	return (1);
}
